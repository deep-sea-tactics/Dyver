...
test_t TEST_AMP_DISTRIBUTOR = test_t("test_amp_distributor", __LINE__, []()
{
    ...
    std::shared_ptr<dynamic_amp_request_t> req_a = distributor.invoke_request(6.0, AMP_REQUEST_PRIORITY::ALWAYS_FULFILL);
    std::shared_ptr<dynamic_amp_request_t> req_b = distributor.invoke_request(3.0, AMP_REQUEST_PRIORITY::DISTRIBUTE);
    std::shared_ptr<dynamic_amp_request_t> req_c = distributor.invoke_request(1.0, AMP_REQUEST_PRIORITY::DISTRIBUTE);
